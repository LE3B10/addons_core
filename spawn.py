...
class MYADDON_OT_create_spawn_point(bpy.types.Operator):
    ...
    archetype: EnumProperty(name="Archetype", items=ENEMY_ARCHETYPE_ITEMS, default="RifleGrunt")
    ...
    def draw(self, context):
        ...
        if self.spawn_type == "EnemySpawnPoint":
            col.prop(self, "archetype")
    ...
    def execute(self, context):
        ...
        # 既存props掃除
        for k in ("wave", "group", "count", "archetype"):
            if k in new_obj:
                del new_obj[k]
        ...
        # EnemySpawnPoint の場合、archetype を設定
        if self.spawn_type == "EnemySpawnPoint":
            new_obj["archetype"] = self.archetype
